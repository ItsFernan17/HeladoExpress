...
const RegistroModal: React.FC<RegistroModalProps> = ({
  ...
  // Función para manejar cambios en inputs con validación
  const handleInputChange = (name: string, value: any) => {
    ...
    if (field?.type === 'number') {
      ...
      if (isNaN(numValue) || numValue < 0) {
        showWarning(`${field.label} debe ser un número positivo`);
        return;
      }
      if (field.min !== undefined && numValue < field.min) {
        showWarning(`${field.label} debe ser mayor o igual a ${field.min}`);
        return;
      }
      ...
      if (textValue.length > 0 && textValue.length < 2) {
        showWarning(`${field.label} debe tener al menos 2 caracteres`);
      }
      
      // Validar longitud máxima
      if (textValue.length > 50) {
        showWarning(`${field.label} no puede exceder los 50 caracteres`);
        return;
      }
      
      // Validar si es requerido
      if (field.required && !textValue) {
        showWarning(`${field.label} es obligatorio`);
      }
      
      setFormData((prev: any) => ({ ...prev, [name]: textValue }));
    } else {
